 ...
 */
 ...
 function jobs_config()
 {
 	$configArray = array(
 		...
 		"language" => "english",
 		"fields" => array(
 			"hremail" => array("FriendlyName" => "Contact Email", "Type" => "text", "Size" => "25", "Description" => "Enter a contact email here.",
 				"Default" => "humanresoucres@example.com"),
 			"homeTab" => array("FriendlyName" => "Home Tab Text", "Type" => "text", "Size" => "25", "Description" => "Enter the text for the home tab here.",
 				"Default" => "Home")
 			)
 	);
 ...
 {
 	$query = "CREATE TABLE  `jobs_joblist` (
			 `job_id` INT(255) NOT NULL PRIMARY KEY AUTO_INCREMENT,
			 `job_title` VARCHAR(255),
			 `job_reference` VARCHAR(25),
			 `job_description` TEXT,
			 `job_department` TEXT,
			 `job_requirments` TEXT,
       `job_salary` TEXT,
			 `job_active` INT(25)
			)";
	...
	$query = "CREATE TABLE `jobs_applicants`
			(
				`app_id` int(255) NOT NULL PRIMARY KEY AUTO_INCREMENT,
				`app_fname` varchar(255),
				`app_lname` varchar(255),
				`app_address` text,
	...
	$query = "CREATE TABLE `jobs_interviews`
			(
				`inter_id` int(255) NOT NULL PRIMARY KEY AUTO_INCREMENT,
				`inter_appid` int(255),
				`inter_fname` varchar(255),
				`inter_lname` varchar(255),
				`inter_date` datetime,
				`inter_admin` int(25),
				`inter_trans` text,
				`inter_notes` text
	...
	$resultThree = mysql_query($query);

	$query = "CREATE TABLE `jobs_settings`
			(
				`setting_id` int(255) NOT NULL PRIMARY KEY AUTO_INCREMENT,
				`setting_val` text
			)";

	$resultFour = mysql_query($query);

	$resultFive = insert_query("jobs_settings", array("setting_val" => "Put your welcome text here."));
	//$resultSix = insert_query("jobs_settings", array("setting_val" => "Home"));

	if ($result === TRUE && $resultTwo === TRUE && $resultThree === TRUE && $resultFour === TRUE)
	{
...
{
	$query = "DROP TABLE `jobs_joblist`";
	$result = mysql_query($query);

	$query = "DROP TABLE `jobs_applicants`";
	$resultTwo = mysql_query($query);

	$query = "DROP TABLE `jobs_interviews`";
	$resultThree = mysql_query($query);

	$query = "DROP TABLE `jobs_settings`";
	$resultFour = mysql_query($query);
